...
namespace ORB_SLAM3 {
    ...
    bool EdgeSE3ProjectXYZOnlySLPoseToBody::read(std::istream& is){
        for (int i=0; i<2; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<2; i++)
    ...
    bool EdgeSE3ProjectXYZOnlySLPoseToBody::write(std::ostream& os) const {
    ...
    void EdgeSE3ProjectXYZOnlySLPoseToBody::linearizeOplus() {
        g2o::VertexSE3Expmap * vi = static_cast<g2o::VertexSE3Expmap *>(_vertices[0]);
        g2o::SE3Quat T_lw(vi->estimate());
        Eigen::Vector3d X_l = T_lw.map(Xw);
        Eigen::Vector3d X_sl = mTsll.map(T_lw.map(Xw));
        ...
        _jacobianOplusXi = -pCamera->projectJac(X_sl) * mTsll.rotation().toRotationMatrix() * SE3deriv;
    }

    bool EdgeSE3ProjectXYZOnlySRPoseToBody::read(std::istream& is){
        for (int i=0; i<2; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<2; i++)
    ...
    bool EdgeSE3ProjectXYZOnlySRPoseToBody::write(std::ostream& os) const {
    ...
    void EdgeSE3ProjectXYZOnlySRPoseToBody::linearizeOplus() {
        g2o::VertexSE3Expmap * vi = static_cast<g2o::VertexSE3Expmap *>(_vertices[0]);
        g2o::SE3Quat T_lw(vi->estimate());
        Eigen::Vector3d X_l = T_lw.map(Xw);
        Eigen::Vector3d X_sr = mTsrl.map(T_lw.map(Xw));
        ...
        _jacobianOplusXi = -pCamera->projectJac(X_sr) * mTsrl.rotation().toRotationMatrix() * SE3deriv;
    }
    ...
    void EdgeSE3ProjectXYZToBody::linearizeOplus() {
     ...
     EdgeSE3ProjectXYZSLToBody::EdgeSE3ProjectXYZSLToBody() : BaseBinaryEdge<2, Eigen::Vector2d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>() {
     }

     bool EdgeSE3ProjectXYZSLToBody::read(std::istream& is){
         for (int i=0; i<2; i++){
             is >> _measurement[i];
         }
         for (int i=0; i<2; i++)
     ...
     bool EdgeSE3ProjectXYZSLToBody::write(std::ostream& os) const {
     ...
     void EdgeSE3ProjectXYZSLToBody::linearizeOplus() {
         g2o::VertexSE3Expmap * vj = static_cast<g2o::VertexSE3Expmap *>(_vertices[1]);
         g2o::SE3Quat T_lw(vj->estimate());
         g2o::SE3Quat T_slw = mTsll * T_lw;
         g2o::VertexSBAPointXYZ* vi = static_cast<g2o::VertexSBAPointXYZ*>(_vertices[0]);
         Eigen::Vector3d X_w = vi->estimate();
         Eigen::Vector3d X_l = T_lw.map(X_w);
         Eigen::Vector3d X_sl = mTsll.map(T_lw.map(X_w));

         _jacobianOplusXi =  -pCamera->projectJac(X_sl) * T_slw.rotation().toRotationMatrix();
         ...
         _jacobianOplusXj = -pCamera->projectJac(X_sl) * mTsll.rotation().toRotationMatrix() * SE3deriv;
     }

     EdgeSE3ProjectXYZSRToBody::EdgeSE3ProjectXYZSRToBody() : BaseBinaryEdge<2, Eigen::Vector2d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>() {
     }

     bool EdgeSE3ProjectXYZSRToBody::read(std::istream& is){
         for (int i=0; i<2; i++){
             is >> _measurement[i];
         }
         for (int i=0; i<2; i++)
     ...
     bool EdgeSE3ProjectXYZSRToBody::write(std::ostream& os) const {
     ...
     void EdgeSE3ProjectXYZSRToBody::linearizeOplus() {
         g2o::VertexSE3Expmap * vj = static_cast<g2o::VertexSE3Expmap *>(_vertices[1]);
         g2o::SE3Quat T_lw(vj->estimate());
         g2o::SE3Quat T_srw = mTsrl * T_lw;
         g2o::VertexSBAPointXYZ* vi = static_cast<g2o::VertexSBAPointXYZ*>(_vertices[0]);
         Eigen::Vector3d X_w = vi->estimate();
         Eigen::Vector3d X_l = T_lw.map(X_w);
         Eigen::Vector3d X_sr = mTsrl.map(T_lw.map(X_w));

         _jacobianOplusXi =  -pCamera->projectJac(X_sr) * T_srw.rotation().toRotationMatrix();
         ...
         _jacobianOplusXj = -pCamera->projectJac(X_sr) * mTsrl.rotation().toRotationMatrix() * SE3deriv;
     }
