...
We release a compact version of the code with the primary features integrated into ORB-SLAM3 to demonstrate the concept, some functions introduced in the paper such as data pre-processing and IMU intrinsic compensation are not included. **Please be advised that this is a re-implemented version based on ORB_SLAM3 and not the original MAVIS code. The original code will not be released.**
...
Clone the git repo from here [euroc_dataset_tools](https://github.com/1fwang/euroc_dataset_tools)
```
python rosbag2euroc.py /PATH/TO/YOUR/file.bag -o /YOUR/OWN/DIR/
```
